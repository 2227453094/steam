...
function getPage(data){
    ...
    for(let i = 0; i < data.length; i++){
        ...
        A.mouseenter(function () {
            ...
            let evaluateList = data[$(this).index()].evaluate;
            if (evaluateList == 1) {
                title_lable.html(`好评如潮`);
                title_lable.css("color", "#66C0F4;");
            } else if (evaluateList == 2) {
        ...
        if(data[i].price == "免费开玩" || data[i].price == "免费"){
             A.find(".tab_item_discount > .discount_final_price").html(data[i].price);
        }
        if(data[i].discount == 0){
            A.find(".tab_item_discount > #discount_pct").html("");
        }
        if(data[i].originPrice == "免费开玩" || data[i].originPrice == "免费"){
            A.find(".tab_item_discount > .discount_original_price").html("");
            A.find(".tab_item_discount > .discount_final_price").html(`${data[i].price}`);
        }
        if(data[i].originPrice == "免费开玩" && data[i].price == "免费开玩"){
            A.find(".tab_item_discount > .discount_final_price").html(`${data[i].price}`);
        }
...
$(".pagebtn_zuo").click(function () {
    ...
    if(index == 0){
        ...
        $(spanchuan[index - 1]).css("color","#fff");
        $("#items").empty();
    }
});


$(".pagebtn_you").click(function(){
    spanchuan.css("color","#3b6e8c");
    index++;
    let src =  $("<script>");
    let head = $("head");
...
//正在实况直播的游戏
let store_capsule_a = $(".store_capsule_a");
let a_hover = $(".a_hover");
let img_move;
store_capsule_a.mouseenter(function () {
    a_hover.css("display","none");
    $(a_hover[$(this).index()]).css({
        "z-index":99,
        "top":"60px",
        "left":"316px",
        "display":"block"
    });
    let ulS = $(".store_capsule_a").find(".a_hover > .dakuai > .posi");
    let this_img = $(ulS[$(this).index()]).find("li > img");
    let index  = 0;
    img_move = setInterval(function () {
        index++;
        this_img.fadeIn(1000);
        $(this_img[index]).fadeOut(1000);
        if(index == 4){
            index = 0;
        }
    }, 16);
});
store_capsule_a.mouseleave(function () {
    a_hover.css("display","none");
